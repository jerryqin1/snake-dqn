# baseline environment definition can be found at the following repository:
...
SLEEP = 0.01  # time to wait between steps; make this smaller for faster training
...
class Snake(gym.Env):
    def __init__(self, human=False, env_info={'state_space': 'only_walls'}):
    ...
    # generates new random location for apple
    ...
    # inrements game score after eating apple
    ...
    # gets distance between snake head and apple
    def measure_distance(self):
        self.prev_dist = self.dist
        self.dist = math.sqrt(
            (self.snake.xcor() - self.apple.xcor()) ** 2 + (self.snake.ycor() - self.apple.ycor()) ** 2)

    # checks if snake has run into self
    ...
    # checks if snake has eaten apple
    ...
    # runs a single time step of game
    def run_game(self):
        reward_given = False
        self.win.update()
        self.move_snake()
        ...
        if self.body_check_snake():
            self.reward = -2000 # modify this to promote snake not crashing into self [100, 175, 200]
            reward_given = True
            self.done = True
            if self.human:
                self.reset()
    ...
    # runs a single time step of game, used for rl agent
    ...
    # gets current state conditioned on state space representation
    def get_state(self):
        ...
        if self.env_info['state_space'] == 'coordinates':
        ...
        elif self.env_info['state_space'] == 'no_direction':
            state = [int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x),
                     ...
                     int(wall_left or body_left), \
                     0, 0, 0, 0]
        elif self.env_info['state_space'] == 'no_body_knowledge':
            state = [int(self.snake.y < self.apple.y), int(self.snake.x < self.apple.x),
                     int(self.snake.y > self.apple.y), int(self.snake.x > self.apple.x), \
